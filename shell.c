...
int running_under_emacs;//这个解释bash的emacs模式与之对应的就是vi模式
...
struct {
  char *name;
  int type;
  int *int_value;//如果% 对应的是整数则存储在这里
  char **char_value;//这个参数就是%s等等后面接的字符串
} long_args[] = {
...
// -s 选项用于指定 Bash 在执行脚本时，将其标准输入与脚本的输入进行连接，从而使脚本能够从标准输入读取数据。这通常用于接收用户的输入或从管道中读取数据。
// 示例：bash -s < input.txt，其中脚本从标准输入 (input.txt) 读取数据。
// -c 选项：

// -c 选项用于在命令行中指定要执行的命令字符串。它允许你将一个命令作为参数传递给 Bash，并且 Bash 将执行该命令。
// 示例：bash -c "echo Hello"，其中 Bash 将执行 echo Hello 这个命令。
...
#endif /* !NO_MAIN_ENV_ARG */
{
  ...
  check_dev_tty ();//检查当前进程是否在一个终端（tty）上运行
  ...
  set_default_locale ();//设置好本地环境, 地理上的

  running_setuid = uidget ();//获取用户的id

  posixly_correct = (getenv ("POSIXLY_CORRECT") != (char *)NULL) ||
		    (getenv ("POSIX_PEDANTIC") != (char *)NULL);//检查关于标准的问题

#if defined (USE_GNU_MALLOC_LIBRARY)
  mcheck (programming_error, (void (*) ())0);
#endif /* USE_GNU_MALLOC_LIBRARY */

  if (setjmp (subshell_top_level))//之后longjmp()函数会跳到这里 顶级子shell
    {
      argc = subshell_argc;
      argv = subshell_argv;
      env = subshell_envp;
  ...
  default_input = stdin;
#if defined (BUFFERED_INPUT)
  default_buffered_input = -1;//shell从这个文件中获取数据
#endif
  ...
  shell_start_time = NOW;	/* NOW now defined in general.h 设置shell启动时间*/

  /* Parse argument flags from the input line. */

  /* Find full word arguments first. */
  arg_index = parse_long_options (argv, arg_index, argc);//通过长参数选项进行设置
  ...
  arg_index = parse_shell_options (argv, arg_index, argc);//这里是通过短的参数来进行设置如-o -s等等
      ...
      for (i = 0; long_args[i].name; i++)
	{
	  if (STREQ (arg_string + 1, long_args[i].name))
		...
		*long_args[i].int_value = 1;
	      else if (argv[++arg_index] == 0)//这里意思是没有带参数
		{
		...
		*long_args[i].char_value = argv[arg_index];//带了参数, 将这些对应的字符串存起来. 

	      break;
	    }
	}
      if (long_args[i].name == 0)//遇到了一个没有在long_args中定义的参数
	{
	  if (longarg)
	    {
...
shell_initialize ()
{
  ...
  if (shell_initialized == 0)//如果没有被初始化过
    {
  ...
  initialize_traps ();//初始化陷阱
  initialize_signals ();

  /* It's highly unlikely that this will change. */
  if (current_host_name == 0)//获取主机的命令
    {
  ...
  /* Initialize the stuff in current_user that comes from the password
     ...
     interactive. */
  if (interactive_shell)
    get_current_user_info ();//初始化用户信息, 根据password文件

  /* Initialize our interface to the tilde expander. */
  tilde_initialize ();//初始化波浪线接口

  /* Initialize internal and environment variables.  Don't import shell
     functions from the environment if we are running in privileged or
     restricted mode or if the shell is running setuid. */
#if defined (RESTRICTED_SHELL)
  initialize_shell_variables (shell_environment, privileged_mode||restricted||running_setuid);//变量初始化
#else
  ...
  initialize_job_control (0);//作业初始化

  /* Initialize input streams to null. */
  initialize_bash_input ();//输入流初始化
...
#if defined (RESTRICTED_SHELL)
  initialize_shell_options (privileged_mode||restricted||running_setuid);//初始化shell设置
#else
...
#if defined (HISTORY)
  bash_history_reinit (0);//初始化bash的历史记录功能
#endif /* HISTORY */
