...
/* The value of $$. */
int dollar_dollar_pid;//$$ 表示当前进程的pid
     ...
     int search_tempenv;
{
  ...
  if ((search_tempenv || subshell_environment) &&
    ...
    var = find_tempenv_variable (name);//首先在临时变量中查找 变量名，如果没有找到就会执行下面一个if语句

  if (!var)
    var = var_lookup (name, shell_variables);//在这个哈希表中再查找一次,如果找到了目标则不会执行下一个if语句
     ...
     char *var_name;
{
  ...
  var = find_variable (var_name);//首先去搜索, 获取这个变量的值
     ...
     char **array;
{
  ...
  for (i = 0, l = strlen (name); array[i]; i++)
      ...
      if (STREQN (array[i], name, l) && array[i][l] == '=')
	{
	  ...
	  temp = new_shell_variable (name);//创建一个新的变量
	  w = array[i] + l + 1;
     ...
     char *name;
{
  ...
  if (!var && builtin_env)//临时环境没有, 内嵌函数环境
    var = find_name_in_env_array (name, builtin_env);

  if (!var && variable_context && function_env)//还是没有并且存在变量上下文的话, 去搜索函数环境
