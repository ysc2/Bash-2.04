...
} BUCKET_CONTENTS;//桶结构体, 其实就是hash中存储的内容

typedef struct hash_table {
  BUCKET_CONTENTS **bucket_array;	/* Where the data is kept. */
  int nbuckets;			/* How many buckets does this table have. 这个表中有多少桶*/
  int nentries;			/* How many entries does this table have. 其实就是表中存储了多少了键值对*/
} HASH_TABLE;//hash表结构体
