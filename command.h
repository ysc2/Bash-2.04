...
enum command_type { cm_for, cm_case, cm_while, cm_if, cm_simple, cm_select,
		    ...
		    cm_arith, cm_cond, cm_arith_for, cm_subshell };//命令类型
...
#define W_GLOBEXP	0x08	/* 通配符 This word is the result of a glob expansion. */
...
/* Possible values for subshell_environment subshel​​l_environment 的可能值*/
...
typedef struct word_desc {
  ...
  int flags;		/* Flags associated with this word. 这个flags就是上面所定义的W_HASDOLLAR等等*/
} WORD_DESC;//命令等各种结构都可能引用字符串或字符串数组。bash对于有必要做进一步语义处理的字符串使用WORD_DESC、WORD_LIST结构进行封装。

/* A linked list of words. */
typedef struct word_list {
  struct word_list *next;
  WORD_DESC *word;
} WORD_LIST;//上面结构所组成的列表
