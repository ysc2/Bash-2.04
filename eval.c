...
parse_command ()
{
  ...
  if (interactive && bash_input.type != st_string)
    {
      command_to_execute = get_string_value ("PROMPT_COMMAND");//根据这个字符串的值来获取这个变量的值
      if (command_to_execute)
	execute_prompt_command (command_to_execute);//执行命令, 这个execute_prompt_command()函数是通过文本分析的程序生成的
...
read_command ()
{
  ...
  if (interactive)//这个shell是交互shell
    {
  ...
  current_command_line_count = 0;
  result = parse_command ();//使用yyparse()函数解析命令, 并且返回解析的状态
